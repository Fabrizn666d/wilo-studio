...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main id="contenido" className="inner-page product-page">
      ...
      <section className="product-detail section-cream"><div className="shell"><Link className="back-link" href="/tienda"><ArrowLeft /> Volver a la tienda</Link><div className="product-detail-grid"><div className="product-detail-gallery"><div className="product-detail-media"><Image src={images[0]} alt={`${product.name} — producto original`} fill priority sizes="(max-width: 900px) 100vw, 50vw" /><span>{stockLabel}</span></div>{images.length > 1 && <div className="product-detail-thumbnails" aria-label={`Galería de ${product.name}`}>{images.slice(1).map((image, index) => <div key={image}><Image src={image} alt={`${product.name}, vista ${index + 2}`} fill sizes="(max-width: 680px) 45vw, 15vw" /></div>)}</div>}</div><div className="product-detail-copy"><span className="eyebrow">{product.category}</span><h1>{product.name}</h1><p className="product-license">{product.license}</p><p className="product-description">{product.description}</p><div className="detail-price"><span>{product.price === null ? "Precio por confirmar" : "Precio con IGV"}</span><strong>{product.price === null ? "Cotiza disponibilidad" : formatSoles(product.price)}</strong><small>{stockLabel} · {product.delivery}</small></div><div className="detail-actions"><AddToCart product={product} large /><a className="button button-ghost button-large" href={settings.whatsapp} target="_blank" rel="noreferrer"><MessageCircle /> Consultar</a></div><div className="purchase-reassurance"><span><BadgeCheck /> Licencia original</span><span><ShieldCheck /> Compra segura</span><span><PackageCheck /> Soporte de activación</span></div></div></div></div></section>
