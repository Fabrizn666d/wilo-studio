...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ plan?: string; service?: string }> }) {
  const [{ plan, service }, settings] = await Promise.all([searchParams, getPublicSiteSettings()]);
  return (
    <main id="contenido" className="inner-page contact-page">
      ...
      <section id="cotizador" className="section quote-section section-dark"><div className="shell"><QuoteWizard initialPlan={plan} initialService={service} /></div></section>
    </main>
  );
}
