...
const promotionShape = {
  ...
  code: z.string().trim().toUpperCase().min(3).max(40).regex(/^[A-Z0-9_-]+$/).optional().nullable().or(z.literal("")),
  discountType: z.enum(["PERCENT", "FIXED"]).optional().nullable(),
  discountValue: z.number().int().min(1).max(100_000_000).optional().nullable(),
  minimumCents: money.default(0),
  maxUses: z.number().int().min(1).max(1_000_000).optional().nullable(),
  usedCount: z.number().int().min(0).max(1_000_000).default(0),
  categorySlugs: jsonText.default("[]"),
  productIds: jsonText.default("[]"),
...
export const adminResources = {
  projects: {
    ...
    searchFields: ["title", "slug", "summary", "code"],
  ...
  promotions: {
    ...
    jsonFields: ["details", "categorySlugs", "productIds"],
  },
  referrals: {
